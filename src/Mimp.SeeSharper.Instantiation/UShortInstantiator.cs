...
{
    /// <summary>
    /// A <see cref="IInstantiator"/> to instantiate <see cref="ushort"/> or <see cref="Nullable{ushort}"/>.
    /// </summary>
    public class UShortInstantiator : IInstantiator
    {
        ...
        public UShortInstantiator(IFormatProvider formatProvider, NumberStyles numberStyles)
        {
            FormatProvider = formatProvider ?? throw new ArgumentNullException(nameof(formatProvider));
            NumberStyles = numberStyles;
        }

        public UShortInstantiator(IFormatProvider formatProvider)
            : this(formatProvider, NumberStyles.Integer) { }

        public UShortInstantiator()
        ...
        public object? Instantiate(Type type, IObjectDescription description, out IObjectDescription? ignored)
        {
            ...
            if (description.HasValue)
            {
                ...
                if (description.Value is ushort v)
                {
                    ignored = null;
                    return v;
                }
                ...
                {
                    ignored = null;
                    return (ushort)description.Value;
                }
        ...
        protected virtual object? InstantiateFromString(Type type, string value, IObjectDescription description, out IObjectDescription? ignored)
        {
            if (string.IsNullOrWhiteSpace(value) && type.IsNullable())
            {
                ...
                try
                {
                    var result = ushort.Parse(value, NumberStyles, FormatProvider);
                    ignored = null;
                    return result;
                }
                catch (Exception ex)
